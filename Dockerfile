...
ENV GOARCH=amd64
# config
WORKDIR /go/src/elastic-gpu-exporter
COPY . .
...
RUN go build -o /go/bin/elastic-gpu-exporter cmd/main.go

# runtime image
FROM nvidia/cuda:10.0-base
COPY --from=build /go/bin/elastic-gpu-exporter /usr/bin/elastic-gpu-exporter
RUN ln -s /usr/lib/x86_64-linux-gnu/libnvidia-ml.so.1 /usr/lib/x86_64-linux-gnu/libnvidia-ml.so

CMD ["elastic-gpu-exporter"]